...
void resolve_hostname_open_socket() {
    ...
    servAddr.sin_port = htons(6000); //Port 6000 is assigned
    ...
    remoteAddr.sin_port = htons(5000); //Port 6000 is assigned
...
int main() {
    while (1) {
        puts("Please choose the function:  to send (0), receive (1), or exit (2)?");
        int input;
        scanf("%d", &input);
        if (input == 0) {
